...
int i=0,maxcnt=10;
int stblcnt=0,actstbl=3;
float oldvolt=0;
  while( digitalRead(irPin)==0 && i<maxcnt)
  {
  ...
  for (i=0;i<maxcnt;i++){
    delay(10);
    get_volt();
    if(abs(volt-oldvolt)>0.05)
    {
      oldvolt=volt;
      stblcnt=0; // reset stable count
    }else{
      stblcnt=stblcnt+1;  // increace stable count
      if (stblcnt==actstbl) // is stable reading than exit
        i=maxcnt;
  ...
  //if cover is close wait till open
  if (i>0)
    Serial.println("still Covered");
  i=0;
  delay(10);
  // off_led();
  }
  ...
  //if cover is open wait till close
  delay(10);
  off_led();
  }
